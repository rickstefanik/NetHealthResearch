clear;
studentNum = 7;
filename = ['Predicted_GLMM_AllStudents_SignificantFactors_', num2str(studentNum) '.csv'];
...
filename = ['Actual_GLMM_AllStudents_SignificantFactors_', num2str(studentNum) '.csv'];
[M, flag] = readSecondCol(filename);
if (flag == 0)
    disp(['File could not be found for student ', num2str(studentNum)]);
end
...
title(['GLMM Model of All Students (Only Significant Factors)']);
