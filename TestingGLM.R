...
i = 10
  ...
  # fit <- glm(mydata$Sleep.Duration..hrs. ~ mydata$Acitivty.State.0..min. +
  #            mydata$Activity.State.1..min. + mydata$Activity.State.2..min. +
  #            mydata$Activity.State.3..min. + mydata$Calories.Burned.State.0 +
  #            mydata$Calories.Burned.State.1 + mydata$Calories.Burned.State.2 +
  #            mydata$Calories.Burned.State.3 +
  #            mydata$Screen.Usage..min. + mydata$Nap.Data..hrs. +
  #            mydata$isSaturday + mydata$isFriday + mydata$isThursday +
  #            mydata$isWednesday + mydata$isTuesday + mydata$isMonday +
  #            mydata$isWeekend + 
  #            mydata$isRegular + mydata$isReadingDay + mydata$isReadingDay +
  #            mydata$Mean.Temperature + mydata$Median.Temperature +
  #            mydata$Daily.Audio.Usage..min. + mydata$Audio.Usage.30.min.before.Sleep..min. +
  ...
  #            mydata$Min.to.Fall.Asleep + mydata$Audio.Usage.in.Bed..min. +
  #            mydata$Screen.Usage.in.Bed..min., family = gaussian())
  ...
  sleepPrediction = (fit$b["(Intercept)"] +
     ...
     (fit$b["mydata$Calories.Burned.State.0..min."] * myValidateData$Calories.Burned.State.0..min.[k] ) +
     (fit$b["mydata$Calories.Burned.State.1..min."] * myValidateData$Calories.Burned.State.1..min.[k] ) +
     (fit$b["mydata$Calories.Burned.State.2..min."] * myValidateData$Calories.Burned.State.2..min.[k] ) +
     (fit$b["mydata$Calories.Burned.State.3..min."] * myValidateData$Calories.Burned.State.3..min.[k] ) +
